...
export class Stream<T extends GrpcData, R extends GrpcData> {
  ...
  public addEventListener<K extends StreamEvents>(
    ...
    const eventListener = { listener, options } as GrpcStreamEventsListener<T, R, K>
  ...
  private dispatchEvent<K extends StreamEvents>(type: K, ev: StreamEventMap[K]) {
    ...
    listeners.forEach((listener) => {
      listener.listener(this, ev)
      if (listener.options && (listener.options as AddEventListenerOptions).once) {
        onceListeners.push(listener)
      }
    })

    onceListeners.forEach((listener) =>
      this.removeEventListener(type, listener.listener, listener.options),
    )
  }
