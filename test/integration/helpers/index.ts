...
export const clearAllTables = async (): Promise<void> => {
  ...
  await testDb("stream_cursors").del()
  await testDb("nwc_audit_log").del()
...
export const getStreamCursorByName = async (
  streamName: string,
): Promise<StreamCursorRecord | undefined> => {
  const testDb = getTestDb()
  return testDb("stream_cursors").where({ stream_name: streamName }).first()
}
