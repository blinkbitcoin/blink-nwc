...
describe("translateStatus", () => {
  it("should map SUCCESS to PAID", () => {
    expect(translateStatus("SUCCESS")).toBe(PaymentState.PAID)
  })

  it("should map PAID to PAID", () => {
    expect(translateStatus("PAID")).toBe(PaymentState.PAID)
  })

  it("should map PENDING to PENDING", () => {
    expect(translateStatus("PENDING")).toBe(PaymentState.PENDING)
  })

  it("should map EXPIRED to EXPIRED", () => {
    expect(translateStatus("EXPIRED")).toBe(PaymentState.EXPIRED)
  })

  it("should map FAILURE to FAILED", () => {
    expect(translateStatus("FAILURE")).toBe(PaymentState.FAILED)
  })

  it("should map undefined to UNKNOWN", () => {
    expect(translateStatus(undefined)).toBe(PaymentState.UNKNOWN)
  })

  it("should map unknown string to UNKNOWN", () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    expect(translateStatus("SOMETHING_ELSE" as any)).toBe(PaymentState.UNKNOWN)
  })
})
